...
export default function Navbar() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.timeline({ delay: 1.1, defaults: { ease: 'power3.out' } })
        .to('.nav-logo',  { opacity: 1, y: 0, duration: 0.7 })
        .to('.nav-pill',  { opacity: 1, y: 0, scale: 1, duration: 0.65 }, '-=0.4')
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex justify-between items-center transition-all duration-700 ${isScrolled ? 'h-14' : 'h-16'}`}>
          ...
          <div className="nav-logo flex-shrink-0 flex items-center w-1/4">
            <Link href="/" className="font-serif text-xl sm:text-2xl tracking-widest text-gold-500 font-light flex items-center gap-2">
              <span className="w-6 h-6 border border-gold-500 rounded-full flex items-center justify-center text-xs">S</span>
              <span className="hidden sm:block">SEVEN STARS</span>
            </Link>
          </div>
          ...
          <div className="nav-icons flex items-center justify-end space-x-3 w-1/4">
            ...
            <div className={`transition-all duration-[800ms] ease-[cubic-bezier(0.25,1,0.5,1)] flex items-center overflow-hidden ${
              isScrolled ? 'max-w-[200px] opacity-100 mr-3 translate-x-0' : 'max-w-0 opacity-0 mr-0 translate-x-12 pointer-events-none'
            }`}>
              <Link
                href="#contact"
                className="hidden sm:flex px-6 py-2.5 text-sm font-medium rounded-full bg-gold-600 text-dark-990 hover:bg-gold-500 shadow-[0_0_15px_rgba(202,158,90,0.2)] hover:shadow-[0_0_20px_rgba(202,158,90,0.4)] whitespace-nowrap transition-all duration-300"
              >
                Book Table
              </Link>
            </div>
