...
export default function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 5. CTAs staggered
      .to('.hero-cta', { opacity: 1, y: 0, duration: 0.65, stagger: 0.15 }, '-=0.5')
      ...
      .to('.hero-social', { opacity: 1, y: 0, duration: 0.5, stagger: 0.1 }, '-=0.4');
  ...
  return (
    <section ref={sectionRef} className="relative h-screen flex flex-col justify-between overflow-hidden">
      ...
      <div className="relative z-20 w-full h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col pt-32 pb-8">
        ...
        <div className="flex-1 flex flex-col justify-center w-full">
          <div className="w-full flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 lg:gap-6">
            ...
            <div className="max-w-2xl space-y-6">
              ...
              <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-[5.5rem] font-serif text-white leading-[0.9] tracking-tight drop-shadow-2xl overflow-hidden">
                <span className="hero-h1-line1 block pr-2">A Taste of</span>
                <span className="hero-h1-line2 block italic text-gold-500 font-light pr-4 pb-3">Elegance</span>
              </h1>
            ...
            <div className="hero-desc max-w-sm lg:ml-auto lg:self-end mt-8 lg:mt-0 text-left">
              <p className="text-lg md:text-xl text-dark-100 font-light leading-relaxed">
                Experience luxury dining, an elegant ambience, and unforgettable moments in the heart of the countryside.
              </p>
            </div>
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mt-8">
            ...
            <div className="flex w-max animate-marquee items-center gap-2 hover:[animation-play-state:paused]">
              {[...Array(2)].map((_, i) => (
                <div key={i} className="flex gap-8 items-center pr-4">
                  {['Fine Dining', 'Private Events', 'Craft Cellar', 'Gastro Pub', 'Local Produce'].map(tag => (
                    <span key={tag} className="whitespace-nowrap px-4 py-2 rounded-full backdrop-blur-md text-[12px] text-gray-300 flex items-center gap-2 hover:text-white cursor-pointer transition-colors shadow-lg">
                      {tag} <span className="text-gold-500 text-[13px] opacity-80">❋</span>
                    </span>
                  ))}
                </div>
              ))}
          ...
          <div className="hidden md:flex gap-2">
            <a href="#" className="hero-social w-9 h-9 rounded-full border border-white/10 bg-black/40 backdrop-blur-md flex items-center justify-center text-gray-300 hover:text-white hover:bg-white/20 transition-all shadow-lg">
              <Instagram size={15} />
            </a>
            <a href="#" className="hero-social w-9 h-9 rounded-full border border-white/10 bg-black/40 backdrop-blur-md flex items-center justify-center text-gray-300 hover:text-white hover:bg-white/20 transition-all shadow-lg">
              <Linkedin size={15} />
