      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-20 reveal-section">
          <span className="text-[10px] tracking-[0.4em] text-[#475DB1] uppercase font-medium block mb-4">
            {data.upperTag || "Our Menu"}
          </span>
          <h2 className="text-4xl md:text-5xl font-serif text-slate-900">
            {data.heading || "Classic Foundations,"} <br />
            <em className="text-[#475DB1] font-light">{data.headingHighlight || "Global Inspirations"}</em>
          </h2>
        </div>
          ...
          {pillars.map((item: any, idx: number) => {
            ...
            return (
              <div key={idx} className="feature-card relative overflow-hidden p-10 rounded-[2rem] shadow-xl transition-all duration-500 group text-white min-h-[480px]">
                <Image
                  src={item.image || "/images/gallery/feature-classic-1.jpg"}
                  alt={item.title}
                  fill
