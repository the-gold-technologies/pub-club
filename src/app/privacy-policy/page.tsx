...
export default function PrivacyPolicyPage() {
  ...
  useEffect(() => {
    fetchPage("privacy-policy").catch((err) => {
      console.warn("Could not load Privacy Policy from CMS.", err);
    });
  }, [fetchPage]);
  ...
  const contentData = pageData?.sections?.PrivacyPolicyContent || {
  ...
  const loading = isLoading["privacy-policy"] ?? true;
        ...
        <div ref={bgRef} className="absolute inset-0 z-0 scale-110">
          {contentData.backgroundImage && (
            <Image
              ...
              alt={contentData.title || "Privacy Policy"}
              fill
